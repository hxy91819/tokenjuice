...
function buildLargeDocumentSummary(lines: string[], rawText: string): { lines: string[]; compaction: CompactionMetadata } {
  ...
  const headingClips = headings.map((line) => clipMiddleWithHash(line, 180));
  const excerptClips = excerpt.lines.map((line) => clipMiddleWithHash(line, 180));

  return {
    lines: [
      `large document summary: ${lines.length} lines, ${countTextChars(rawText)} chars`,
      ...(headingClips.length > 0 ? ["headings:", ...headingClips.map((line) => `- ${line.text}`)] : []),
      "excerpt:",
      ...excerptClips.map((line) => line.text),
    ],
    compaction: mergeCompactionMetadata(
      createCompactionMetadata("inspection-large-document-summary"),
      excerpt.compaction,
      ...headingClips.map((line) => line.compaction),
      ...excerptClips.map((line) => line.compaction),
    ),
  };
}
